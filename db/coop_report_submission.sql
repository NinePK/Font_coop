...
CREATE TABLE `report_submission` (
  ...
  `report_title_thai` varchar(500) NOT NULL,
  `report_title_english` varchar(500) NOT NULL,
  `submission_date` varchar(10) NOT NULL,
  `student_signature_date` varchar(20) NOT NULL,
  `advisor_approval_status` varchar(20) DEFAULT 'pending',
  `advisor_approval_date` varchar(20) DEFAULT NULL,
  `created_at` datetime DEFAULT current_timestamp(),
  `updated_at` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_report_submission_training_idx` (`training_id`),
  CONSTRAINT `fk_report_submission_training` FOREIGN KEY (`training_id`) REFERENCES `training` (`id`)
